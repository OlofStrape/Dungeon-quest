 ...
 */
import { createEntity, addComponent, createPosition, createSprite, createCollider, createInteractable } from '../ecs/components';
...
export class RoomFactory {
    ...
    createCombatRoom(row, onComplete, onFail) {
        const entities = [];
    ...
    createProgressRoom(row, puzzleManager, onComplete, onFail) {
        const entities = [];
        // Register progress goal
        const goal = PuzzleManager.createProgressGoal(`progress_${row.Rum}`, row['Frågor/uppgifter'], onComplete);
        ...
        for (let i = 0; i < stationCount; i++) {
            ...
            addComponent(station, 'interactable', createInteractable(50, () => this.handleProgressStation(station, puzzleManager, goal.id), 'Solve math problem'));
            entities.push(station);
        }
        return entities;
    }
    createChoiceRoom(row, puzzleManager, onComplete, onFail) {
        const entities = [];
        ...
        choices.forEach((choice, index) => {
            ...
            addComponent(door, 'interactable', createInteractable(60, () => this.handleChoiceDoor(door, choice, puzzleManager, onComplete, onFail), `Choose ${choice}`));
            entities.push(door);
        });
        return entities;
    }
    createDragDropRoom(row, puzzleManager, onComplete, onFail) {
        const entities = [];
        // Create drop zones
        ...
        for (let i = 0; i < dropZoneCount; i++) {
            ...
            addComponent(dropZone, 'interactable', createInteractable(80, () => this.handleDropZone(dropZone, puzzleManager, onComplete), 'Drop item here'));
            entities.push(dropZone);
        }
        return entities;
    }
    createSpecialRoom(row, puzzleManager, onComplete, onFail) {
        const entities = [];
        ...
        addComponent(specialElement, 'interactable', createInteractable(60, () => this.handleSpecialElement(specialElement, row, puzzleManager, onComplete), row.Beskrivning || 'Interact'));
        entities.push(specialElement);
        return entities;
    }
    createGrindRoom(row, puzzleManager, onComplete, onFail) {
    ...
    createBossRoom(row, onComplete, onFail) {
        const entities = [];
    ...
    createDefaultRoom(row, onComplete, onFail) {
        // Fallback room type
        return this.createCombatRoom(row, onComplete, onFail);
    }
    // Event handlers
    handleProgressStation(station, puzzleManager, goalId) {
    ...
    handleChoiceDoor(door, choice, puzzleManager, onComplete, onFail) {
    ...
    handleDropZone(dropZone, puzzleManager, onComplete) {
    ...
    handleSpecialElement(element, row, puzzleManager, onComplete) {
