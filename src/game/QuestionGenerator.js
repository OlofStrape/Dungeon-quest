...
function pickOrRoll(p, min, max) {
    if (Array.isArray(p))
        return rollInt(p[0], p[1]);
    if (typeof p === 'number')
        return p;
    if (typeof p === 'object' && p.min != null && p.max != null)
        return rollInt(p.min, p.max);
    return rollInt(min, max);
}
function mcqIfNeeded(mode, prompt, ans, strategy) {
...
function buildDistractors(correct, strategy) {
