...
import { Entity, Position, Velocity, Collider } from '../components';
...
export class MovementSystem {
  ...
  private hasMovementComponents(entity: Entity): boolean {
  ...
  private handlePlayerInput(velocity: Velocity): void {
    // Reset velocity
    velocity.vx = 0;
    velocity.vy = 0;
    ...
    // Apply movement
    if (left) velocity.vx = -this.speed;
    if (right) velocity.vx = this.speed;
    if (up) velocity.vy = -this.speed;
    if (down) velocity.vy = this.speed;
